...
void initFSR(FSRConstants* fsrData, float sharpness)
{
    FsrEasuCon(fsrData->const0, fsrData->const1, fsrData->const2, fsrData->const3,
               fsrData->input_width,  fsrData->input_height, // frame render resolution
               fsrData->input_width,  fsrData->input_height, // input container resolution
               fsrData->output_width, fsrData->output_height); // upsacled resolution
...
static std::string buildShader(const std::vector<std::string>& headers, const std::vector<std::string>& filenames, const std::map<std::string, std::string>& defines, std::string out_f) 
{
    ...
    std::cout << "Build shader: " << out_f << std::endl;
    //Wu: write out the shader program
    //std::ofstream ofs;
    //ofs.open(out_f);
    //ofs << out.str();
    //ofs.close();
